...
import SearchBar from '../../komponen/umum/c_bilah_pencarian';
import Button from '../../komponen/antarmuka/c_tombol';
import Card from '../../komponen/antarmuka/c_kartu';
import Modal from '../../komponen/antarmuka/c_modal';
import Badge from '../../komponen/antarmuka/c_lencana';
import InputField from '../../komponen/antarmuka/c_kolom_input';
import { Spacing, FontSize, FontWeight, formatRupiah, formatDate, BorderRadius, Shadow } from '../../tema';
import { LoadingSkeleton, EmptyState } from '../../komponen/umum/c_status';
import AnimatedView from '../../komponen/umum/c_tampilan_animasi';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { useFocusEffect } from '@react-navigation/native';
import { Debt, Customer } from '../../tipe';
...
const DebtsScreen: React.FC = ({ route }: any) => {
    const { warna: colors, ambilDataDasbor: fetchDashboard, pengguna: user } = gunakanApp();
    ...
    // Tangani navigasi tab dari parameter
    useEffect(() => {
        if (route.params?.tab) {
            setActiveTab(route.params.tab);
            // Reset parameter agar tidak stuck saat fokus ulang
        }
    }, [route.params?.tab]);
    ...
    // Bersihkan error saat modal berubah
    useEffect(() => {
    ...
    // ─── Fetch Debts ────
    const ambilDataHutang = useCallback(async () => {
        try {
            setDebtLoading(true);
    ...
    // ─── Fetch Customers ────
    const ambilDataPelanggan = useCallback(async () => {
        try {
    ...
    useFocusEffect(
        useCallback(() => {
            ...
            ambilDataPelanggan();
            fetchDashboard();
        }, [ambilDataHutang, ambilDataPelanggan, fetchDashboard])
    );
    ...
    const bayarHutang = async () => {
        ...
        try {
            ...
            ambilDataHutang();
            ambilDataPelanggan(); // Sinkronkan total hutang di daftar pelanggan
            fetchDashboard(); // Perbarui notifikasi
        } catch (err) {
            showAlert('Error', 'Gagal mencatat pembayaran', 'error');
        }
    };

    const tambahCatatanHutang = async () => {
        const errors: any = {};
    ...
    const sendReminder = async (debt: Debt) => {
        try {
            // Perbarui status di backend terlebih dahulu
    ...
    const handleShareQRIS = async (customer: Customer) => {
        ...
        // Karena base64 tidak bisa dibagikan sebagai file tanpa library tambahan, 
        // we'll send the text and instructions. 
        // Jika berupa URL asli, akan tampil preview.
        await contactWhatsApp(phone, customer.name, message);
    };

    const hapusPelanggan = (customer: Customer) => {
    ...
    const simpanDataPelanggan = async () => {
        const errors: any = {};
        ...
        try {
            ...
            if (isEditingCust && editingCustId) {
                ...
                // Perbarui hutang jika sudah ada dan berubah
                if (editingDebtId && newCustInitialDebt) {
                    ...
                    // Buat hutang baru jika sebelumnya tidak ada
                    await api.post('/debts', {
    ...
    const renderCustomerItem = useCallback(({ item, index }: { item: Customer, index: number }) => (
        <AnimatedView delay={(index % 10) * 80} style={{ marginBottom: Spacing.md }}>
            <TouchableOpacity
                ...
                onLongPress={() => hapusPelanggan(item)}
                delayLongPress={800}
                activeOpacity={0.8}
            >
                        ...
                        <>
                            ...
                            {debts.length === 0 ? (
                                ...
                                <FlatList
                                    ...
                                    refreshControl={<RefreshControl refreshing={debtLoading} onRefresh={ambilDataHutang} />}
                ...
                <View style={{ flex: 1 }}>
                    ...
                    {customerLoading && !customerRefreshing ? (
                        ...
                        <FlatList
                            ...
                            showsVerticalScrollIndicator={false}
                            refreshControl={<RefreshControl refreshing={customerRefreshing} onRefresh={() => { setCustomerRefreshing(true); ambilDataPelanggan(); }} colors={[colors.primary]} />}
                        />
                    )}
                </View>
            )}
                ...
                <View style={{ paddingBottom: 10 }}>
                    {selectedDebt ? (
                        <View>
                            ...
                            <Button title="Simpan Pembayaran" onPress={bayarHutang} style={{ marginTop: 20, marginBottom: 10 }} />
                        </View>
                    ) : <View />}
                </View>
            </Modal>
