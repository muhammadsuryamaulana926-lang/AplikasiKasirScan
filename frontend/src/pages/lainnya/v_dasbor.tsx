...
import { LoadingScreen, EmptyState } from '../../komponen/umum/c_status';
...
const DashboardScreen: React.FC = ({ navigation }: any) => {
    const { warna: colors, ambilDataDasbor: fetchDashboard, dataDasbor: dashboardData } = gunakanApp(); // Removed loading usage to handle pull-refresh better
    const [sedangRefresh, setSedangRefresh] = useState(false);

    const tarikUntukRefresh = useCallback(async () => {
        setSedangRefresh(true);
        await fetchDashboard();
        setSedangRefresh(false);
    }, [fetchDashboard]);

    // Perbarui data setiap kali layar difokuskan
    useFocusEffect(
        useCallback(() => {
            fetchDashboard();
        }, [fetchDashboard])
...
const styles = StyleSheet.create({
    ...
    actionIcon: {
        width: 48,
        height: 48,
        borderRadius: BorderRadius.lg, // Kotak melengkung gaya modern
        alignItems: 'center',
        justifyContent: 'center',
    },
    actionLabel: {
