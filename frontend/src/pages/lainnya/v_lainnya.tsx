...
const MoreScreen: React.FC = ({ navigation }: any) => {
    const { warna: colors, pengguna: user, perbaruiPengguna: updateUser, aturSudahMasuk: setIsLoggedIn, tema: theme, aturTema: setTheme } = gunakanApp();
    ...
    const pickImage = async (type: 'avatar' | 'banner' | 'qris') => {
        ...
        if (!result.canceled && result.assets[0].base64) {
            ...
            simpanFotoProfil(type, base64Img);
        }
    };

    const simpanFotoProfil = async (type: 'avatar' | 'banner' | 'qris', uri: string) => {
        if (!user?.id) return;
        setUploading(true);
        try {
            ...
            >
                ...
                <AnimatedView delay={500} style={styles.sectionContainer}>
                    ...
                    <View style={styles.settingsList}>
                        ...
                        <SettingItem icon={<LogOut />} label="Keluar" isDestructive onPress={keluarAkun} />
                    </View>
                </AnimatedView>
...
const styles = StyleSheet.create({
    ...
    fixedContentSheet: {
        ...
        paddingTop: 0, // Padding ditangani oleh margin view internal
        paddingBottom: 10,
        zIndex: 5,
        // This needs to be tall enough or let the scroll content perform visually.
        // View ini hanya menampung item yang tetap (fixed).
    },
    editBannerBtn: {
        position: 'absolute',
        right: 16,
