...
const CustomersScreen: React.FC = ({ navigation }: any) => {
    ...
    const ambilDataPelanggan = useCallback(async () => {
        try {
            ...
            const res = await api.get('/customers', { params: { search: kataCari } });
    ...
    const renderItemPelanggan = useCallback(({ item, index }: { item: Customer, index: number }) => (
