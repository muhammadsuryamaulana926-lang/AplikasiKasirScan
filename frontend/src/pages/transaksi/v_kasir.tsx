...
const POSScreen: React.FC = () => {
    const { warna: colors, tambahKeKeranjang: addToCart, keranjang: cart, totalKeranjang: cartTotal, kosongkanKeranjang: clearCart, ubahJumlahKeranjang: updateCartQty, hapusDariKeranjang: removeFromCart, ambilDataDasbor: fetchDashboard, pengguna: user } = gunakanApp();
    ...
    const ambilPelangganPilihan = useCallback(async () => {
        try {
    ...
    useEffect(() => {
        if (isCustomerModalVisible) {
            ambilPelangganPilihan();
        }
    }, [ambilPelangganPilihan, isCustomerModalVisible]);
    ...
    const ambilDataKasir = useCallback(async () => {
        try {
            setProsesMemuat(true);
            const [prodRes, catRes] = await Promise.all([
                api.get('/products', { params: { search: kataCari, category: selectedCategory, limit: 100 } }),
    ...
    const handleCloseSuccess = () => {
        ...
        setKataCari(''); // Kosongkan pencarian agar stok terbaru tampil
    };
