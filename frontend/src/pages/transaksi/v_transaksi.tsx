...
import { Spacing, FontSize, FontWeight, formatRupiah, formatDateTime, BorderRadius, Shadow } from '../../tema';
import { LoadingSkeleton, EmptyState } from '../../komponen/umum/c_status';
import AnimatedView from '../../komponen/umum/c_tampilan_animasi';
import { Transaction } from '../../tipe';
...
const TransactionsScreen: React.FC = ({ navigation }: any) => {
    ...
    const ambilDataTransaksi = useCallback(async () => {
        try {
            setProsesMemuat(true);
            const res = await api.get('/transactions', { params: { search: kataCari, limit: 100 } });
    ...
    const renderItemTransaksi = useCallback(({ item, index }: { item: Transaction, index: number }) => {
    ...
    return (
        <View style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            {prosesMemuat && !sedangRefresh ? (
                ...
                <FlatList
                    data={transactions}
                    renderItem={renderItemTransaksi}
